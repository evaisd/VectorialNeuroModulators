...
class MFSweepRunner(SweepRunner):
    ...
    def _single_run(self, sweep_param_val: float | np.ndarray, nu_init: np.ndarray, *args, **kwargs):
        ...
        base_runner = MainMFRunner(**self.params)
...
class EffectiveMFSweepRunner(MFSweepRunner):
    ...
    def _single_run(
            self,
            sweep_param_val: float | np.ndarray,
            nu_init: np.ndarray,
    ):
        ...
        self._params_tweaker(sweep_param_val)
        base_runner = MainMFRunner(**self.params)
        mesh, nu_outs = base_runner.run_effective_rates_on_grid(self.focus_pops,
                                                                *self.nu_vecs)
        ...
        return np.concat([potential[peaks], base_runner.ext_mu[:1], base_runner.ext_sigma[:1]])
